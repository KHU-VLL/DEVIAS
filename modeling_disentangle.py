...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.unified_head :
            ...
            self.scene_head = nn.Linear(embed_dim, self.num_scene_classes) if self.num_scene_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x,return_attn=False):
        ...
        scene_tokens = self.scene_token.expand(x.size(0), -1, -1)
        x = torch.cat((cls_tokens, x, scene_tokens), dim=1)
        ...
        x = self.norm(x) 
        # x = self.fc_norm(x) # design like videoMAE
...
@register_model
def disentangle_vit_base_patch16_224(pretrained=False, **kwargs):
