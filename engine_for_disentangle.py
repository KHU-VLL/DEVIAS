    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            loss, output,loss_dict = train_class_batch(
                    model,scene_model, samples, targets, train_criterion)
...
def validation_one_epoch(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 50, header):
        ...
        # compute output
        with torch.cuda.amp.autocast():
            action_outputs, scene_outputs = model(videos)
            output = action_outputs[1]
...
@torch.no_grad()
def final_test_with_scene_label(data_loader, model, scene_model, device, file, num_labels=400, unified_head=False):
    criterion = torch.nn.CrossEntropyLoss()
