...
class VisionTransformer(nn.Module):
    ...
    def get_select_slot_info(self) :
        txt = " | ".join([str(self.select_slots_info[i][0]) for i in range(len(self.select_slots_info))])
        print(f"action slot : {txt}")
        txt = " | ".join([str(self.select_slots_info[i][1]) for i in range(len(self.select_slots_info))])
        print(f"scene slot : {txt}")
    
    def reset_select_slot_info(self) :
        self.select_slots_info = [[0,0] for i in range(self.num_slots)]
...
@register_model
def slot_vit_base_patch16_224(pretrained=False, **kwargs):
