...
class TrainLoss(nn.Module):
    ...
    """
    def __init__(self,
                 criterion:torch.nn.Module, scene_criterion:torch.nn.Module, num_action_classes:int, slot_matching_method='matching',
    ...
    def forward(self, model, student_output, teacher_outputs, target, fg_mask=None):
        if self.slot_matching_method == 'hard_select':
            _, (action_output, _, attn), (slots_head, slots, mask_predictions) = student_output
            bs = target.shape[0]
            device = action_output.device
