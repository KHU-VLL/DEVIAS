...
class FAME(nn.Module):
    ...
    def forward(self, videos, action_label, scene_label):
        ...
        #! refine for HVU (has both action label & scene label)
        video_fuse_scene_labels = scene_label[index]
        ...
        if self.prob_aug < 1:
            ...
            all_action_label = torch.cat([action_label[aug_ind], action_label[ori_ind]], dim=0).contiguous()
            all_scene_label = torch.cat([video_fuse_scene_labels[aug_ind], scene_label[ori_ind]], dim=0).contiguous()
            ...
            all_action_label = action_label
            all_scene_label = scene_label
                
        pooled_data = F.avg_pool2d(mask.squeeze(1).squeeze(1), kernel_size=16, stride=16)
        reshaped_data = pooled_data.view(batch_size, -1)
        mask = reshaped_data.to(action_label.device, non_blocking=True)
        
        pooled_data = F.avg_pool2d(masks_per_frame.squeeze(2), kernel_size=16, stride=16)
        reshaped_data = pooled_data.view(batch_size, -1)
        masks_per_frame = reshaped_data.to(action_label.device, non_blocking=True)
        
        
        return all_videos, all_action_label, all_scene_label, (mask, masks_per_frame)
